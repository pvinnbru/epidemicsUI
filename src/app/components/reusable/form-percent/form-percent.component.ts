...
@Component({
  selector: 'app-form-percent',
  templateUrl: './form-percent.component.html',
  styleUrls: ['./form-percent.component.scss'],
  standalone: true,
  imports: [MatFormFieldModule, MatInputModule, FormsModule, ReactiveFormsModule, CommonModule],
})
export class FormPercentComponent {
  ...
  getErrorMessage(control: AbstractControl): string {
    ...
    if (control === this.numberInRange && control.hasError('numberRange')) {
      return 'Not a number between 0 and 1';
    }
    return '';
  }
  ...
  numberRangeValidator(control: AbstractControl): ValidationErrors | null {
    ...
    } else if (value === '' || isNaN(value) || !(0 <= value && value <= 1)) {
