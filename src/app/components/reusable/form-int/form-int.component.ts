...
export class FormIntComponent implements OnInit {
  ...
  numberRangeValidator(control: AbstractControl): ValidationErrors | null {
    ...
    if (value === '' && !this.isRequired) {
      ...
      value === '' ||
      isNaN(parseInt(value, 10)) || // Check if it's not a valid integer
      !Number.isInteger(Number(value)) || // Check if it's not a valid integer
      !(this.min <= parseInt(value, 10) && parseInt(value, 10) <= this.max)
    ) {
